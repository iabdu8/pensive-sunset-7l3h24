...
export default function App() {
        ...
        if (dist) {
          ...
          if (!tempDistricts[dist].lat) {
            try {
              ...
              if (json[0]) { tempDistricts[dist].lat = parseFloat(json[0].lat); tempDistricts[dist].lng = parseFloat(json[0].lon); }
            } catch (err) {}
          }
  ...
  return (
    <div ref={fullScreenRef} style={{ height: "100vh", width: "100vw", background: "#000", position: "fixed", top: 0, left: 0, direction: "rtl", fontFamily: "sans-serif" }}>
      ...
      {/* اسم الموقع الجديد في الزاوية العلوية */}
      ...
      <div id="action-buttons" style={{ position: "absolute", top: "15px", width: "100%", zIndex: 9999, display: "flex", justifyContent: "center", gap: "10px" }}>
        <label style={{ background: "#2563eb", color: "#fff", padding: "10px 20px", borderRadius: "30px", fontWeight: "bold", cursor: "pointer", fontSize: "13px", boxShadow: "0 4px 15px rgba(37,99,235,0.4)" }}>
      ...
      <div style={{ 
        position: "absolute", bottom: "25px", right: "15px", zIndex: 9999, 
        width: showReport ? "240px" : "120px", transition: "all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275)"
      }}>
        <button onClick={() => setShowReport(!showReport)} style={{ background: "#1e293b", color: "#00f2ff", border: "1px solid #00f2ff", width: "100%", borderRadius: "10px", padding: "8px", cursor: "pointer", fontSize: "11px", fontWeight: "bold", marginBottom: "8px", boxShadow: "0 4px 10px rgba(0,0,0,0.3)" }}>
          {showReport ? "▼ إخفاء التقرير" : "▲ إظهار التقرير"}
        </button>
        
        {showReport && (
          <div style={{ background: "rgba(10, 15, 30, 0.92)", backdropFilter: "blur(12px)", padding: "15px", borderRadius: "20px", border: "1px solid rgba(255,255,255,0.1)", maxHeight: "50vh", overflowY: "auto", boxShadow: "0 10px 30px rgba(0,0,0,0.5)" }}>
            <div style={{ color: "#94a3b8", fontSize: "10px", letterSpacing: "1px" }}>TOTAL SALES</div>
          ...
          {Object.entries(districtsData).map(([name, data], idx) => (
            data.lat && (
              <React.Fragment key={idx}>
                <Circle center={[data.lat, data.lng]} radius={1300} pathOptions={{ fillColor: "#ff0000", color: "transparent", fillOpacity: 0.35 }} />
                <Marker position={[data.lat, data.lng]} icon={createCountIcon(data.transactions.length)}>
                  <Tooltip direction="top" offset={[0, -20]} opacity={1} permanent>
                    <span style={{fontSize: "10px", fontWeight: "bold", color: "#fff", textShadow: "1px 1px 2px #000"}}>{name}</span>
                  </Tooltip>
                </Marker>
              </React.Fragment>
            )
      ...
      {loading && (
        <div style={{ position: "absolute", top: "75px", left: "50%", transform: "translateX(-50%)", zIndex: 10001, background: "#fbbf24", color: "#000", padding: "8px 25px", borderRadius: "30px", fontSize: "12px", fontWeight: "bold", boxShadow: "0 4px 15px rgba(0,0,0,0.3)" }}>جاري تحليل البيانات...</div>
      )}
    </div>
  );
}
